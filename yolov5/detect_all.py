...
weights_list = ['final_model/mask.pt','final_model/chef_mouse.pt', 'final_model/phone_smoke.pt']
...
def run(
    ...
    reversed_behavior_dict = {v: k for k, v in behavior_dict.items()}
    accept_list = ['no_mask', 'without_hat', 'without_chef_clothes', 'calling', 'mouse']
    behavior_dict2 = {
        'mask': '戴口罩',
        'no_mask': '未戴口罩',
        'hat': '戴厨师帽',
        'without_hat': '未戴厨师帽',
        'chef_clothes': '穿厨师服',
        'without_chef_clothes': '未穿厨师服',
        'calling': '打电话',
        'smoking': '抽烟',
        'open_trashcan': '垃圾桶开盖',
        'close_trashcan': '垃圾桶闭盖',
        'mouse': '老鼠'
    }
    if analysis_config.obj_type != None:
        accept_list = []
        for item in analysis_config.obj_type:
            accept_list.append(reversed_behavior_dict[item])
    ...
    for weight in weights_list:
        model = DetectMultiBackend(weight, device=device, dnn=dnn, data=data, fp16=half)
        model_list.append(model)
        names_list.append(model.names)
    stride, pt = model_list[0].stride, model_list[0].pt
    ...
    while True:
        if webcam:
            ...
            try:
                ...
                with app.app_context():
                    response = session.post(task_host, json=msg)
                task_list[threading.current_thread().name]['taskStatus'] = 3
                task_list[threading.current_thread().name]['errorMsg'] = '视频流异常'
                return
        ...
        for model in model_list:
            ...
            dt_list.append(dt)
        task_list[threading.current_thread().name]['taskStatus'] = 1
        for path, im, im0s, vid_cap, s in dataset:
            image = np.squeeze(im)
            image = np.transpose(image, (1, 2, 0))  # 调整维度顺序，变成 (480, 640, 3)
            ...
            for i, model in enumerate(model_list):
                im_list.append(im)
                with dt_list[i][0]:
                    im_list[i] = torch.from_numpy(im_list[i]).to(model.device)
                    im_list[i] = im_list[i].half() if model.fp16 else im_list[i].float()  # uint8 to fp16/32
                    im_list[i] /= 255  # 0 - 255 to 0.0 - 1.0
                    if len(im_list[i].shape) == 3:
                        im_list[i] = im_list[i][None]  # expand for batch dim

                # Inference
                with dt_list[i][1]:
                    visualize = increment_path(save_dir / Path(path).stem, mkdir=True) if visualize else False
                    pred = model(im_list[i], augment=augment, visualize=visualize)
            ...
            for j, pred in enumerate(pred_list):
                for i, det in enumerate(pred):  # per image
                    ...
                    annotator = Annotator(im0, line_width=line_thickness, example=str(names_list[j]))
                    if len(det):
                        ...
                        for c in det[:, 5].unique():
                            ...
                            s += f"{n} {names_list[j][int(c)]}{'s' * (n > 1)}, "  # add to string
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            if analysis_config.min_object_height is not None:
                                if abs(int(xyxy[1]) - int(xyxy[3])) < analysis_config.min_object_width:
                                    continue
                            if names_list[j][int(cls)] not in accept_list:
                                continue

                            img['area']= [{'score': float(f'{conf:.4f}'), 'coordinatesInfo': {'leftTopX':int(xyxy[0]),  'leftTopY': int(xyxy[1]), 'rightBtmX': int(xyxy[2]), 'rightBtmY': int(xyxy[3])}}]
                            img['msg'] =  behavior_dict2[names_list[j][int(cls)]]
                            img['type'] = str(behavior_dict[names_list[j][int(cls)]])
                            img_list.append(img)
                            # img['data'].append({ 'objectType': behavior_dict[names_list[j][int(cls)]], 'objectName' : names_list[j][int(cls)], 'score' : f'{conf:.4f}','leftTopX':int(xyxy[0]),  'leftTopY': int(xyxy[1]),
                            ...
                            if save_img or save_crop or view_img:  # Add bbox to image
                                ...
                                label = None if hide_labels else (names_list[j][c] if hide_conf else f'{names_list[j][c]} {conf:.2f}')
                                annotator.box_label(xyxy, label, color=colors(c, True))
                            if save_crop:
                                save_one_box(xyxy, imc, file=save_dir / 'crops' / names_list[j][c] / f'{p.stem}.jpg', BGR=True)
            ...
            # message = ''
            # for i, data in enumerate(type_list):
            #     if i == 0:
            #         message = '检测到' + data['objectName']
            #     else:
            #         message = message + ', 检测到' +data['objectName']
            # if message == '':
            #     message = '未检测到类别'
            # img['msg'] = message
            res = {}
            res['data'] = {}
            res['data']['data'] = img_list
            res['data']['capturedTime'] = capturedTime
            res['data']['taskId'] = threading.current_thread().name
            res['data']['timestamp'] = int(time.time() * 1000)
            res['data']['base64'] = image_to_base64(im_bgr)
            # img['resultCd'] = 0
            ...
            if analysis_config.cooldown_duration != 0:
                if cur_time - start_time > analysis_config.cooldown_duration:
                    if len(img_list) != 0:
                        with app.app_context():
                            response = session.post(data_host, json=res)
                        start_time = time.time()
                        img_list = []
                        res = {}
            else:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if names_list[j][int(cls)] not in accept_list:
                            continue
                        img['data'].append({'objectType': behavior_dict[names_list[j][int(cls)]], 'objectName': behavior_dict2[names_list[j][int(cls)]], 'score': f'{conf:.4f}',
